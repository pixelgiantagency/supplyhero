export function initVideoBackground() {
  document.querySelectorAll('[data-video-background-init]').forEach(function (player) {
    var desktopSrc = player.getAttribute('data-player-src');
    var mobileSrc = player.getAttribute('data-player-src-mobile');
    var breakpoint = parseInt(player.getAttribute('data-player-mobile-breakpoint'), 10) || 767;

    // Wird beim Attachen einmalig ausgewertet (passt zum Lazy-Load-Verhalten,
    // reagiert also nicht dynamisch auf Resize nach dem Laden)
    function resolveSrc() {
      if (mobileSrc && window.innerWidth <= breakpoint) return mobileSrc;
      return desktopSrc;
    }
